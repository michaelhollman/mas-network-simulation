...
/**
 * Our custom Repast ContextBuilder, which builds up the context with our projections and fills it with agents. Parameters are parsed from the user, and then we use a ContextBuilderBuilder to apply
...
public class NetworkContextBuilder extends DefaultContext<FileSharingNode> implements ContextBuilder<FileSharingNode> {
    ...
    public Context<FileSharingNode> build(Context<FileSharingNode> context) {
        ...
        ContinuousSpace<FileSharingNode> space = spaceFactory.createContinuousSpace("space", context, new RandomCartesianAdder<FileSharingNode>(), new WrapAroundBorders(), 100, 100);
